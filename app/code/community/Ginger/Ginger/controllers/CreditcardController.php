 ...
 *
 **/
class Ginger_Ginger_CreditcardController extends Mage_Core_Controller_Front_Action
{
    /**
     * @var Ginger_Ginger_Model_creditcard
     */
    ...
    public function paymentAction()
    {
        ...
        try {
            ...
            $description = str_replace('%', $order_id,
                Mage::getStoreConfig("payment/ginger_creditcard/description", $order->getStoreId())
            );
            $return_url = Mage::getUrl('ginger/creditcard/return');
            $customer = $this->_getCustomerData($order);
            $currency = $order->getOrderCurrencyCode();

            if ($this->_creditcard->createOrder($order_id, $amount, $currency, $description, $return_url, $customer)) {
                if (!$order->getId()) {
                ...
                $payment->setIsTransactionClosed(false)
                    ->setGingerOrderId($this->_creditcard->getOrderId());

                // Sets the above transaction
                $order->setPayment($payment);
                $order->setGingerOrderId($this->_creditcard->getOrderId());
                $order->save();

                $payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_AUTH);

                $pendingMessage = Mage::helper('ginger')->__(Ginger_Ginger_Model_creditcard::PAYMENT_FLAG_PENDING);
    ...
    public function webhookAction()
    {
        ...
        if (in_array($input['event'], array("status_changed"))) {
            ...
            $ginger_order = $this->_creditcard->getOrderDetails($ginger_order_id);
                        ...
                        case "completed":
                            ...
                            if ($amount_paid_cents == $order_amount_cents) {
                                ...
                                $order->setState(
                                    Mage_Sales_Model_Order::STATE_PROCESSING,
                                    Mage_Sales_Model_Order::STATE_PROCESSING,
                                    Mage::helper('ginger')->__(Ginger_Ginger_Model_Creditcard::PAYMENT_FLAG_COMPLETED),
                                    true
                                );
                                $order->save();
                                ...
                                if (Mage::getStoreConfig("payment/ginger_creditcard/send_invoice_mail",
                                    $order->getStoreId())
                                ) {
                                ...
                                // Create invoice.
                                if (Mage::getStoreConfig("payment/ginger_creditcard/generate_invoice_upon_completion",
                                    $order->getStoreId())
                                ) {
                                ...
                                $order->setState(Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW,
                                    ...
                                    Mage::helper('ginger')->__(Ginger_Ginger_Model_Creditcard::PAYMENT_FLAG_FRAUD),
                                    false)->save();
                            }
                            break;
                        case "cancelled":
                            ...
                            $order->setState(
                                ...
                                Mage_Sales_Model_Order::STATE_CANCELED,
                                Mage::helper('ginger')->__(Ginger_Ginger_Model_Creditcard::PAYMENT_FLAG_CANCELLED),
                                true
                            );
                            $order->save();
                            break;
    ...
    public function returnAction()
    {
        ...
        try {
            if (!empty($order_id)) {
                $ginger_order_details = $this->_creditcard->getOrderDetails($order_id);
    ...
    protected function _savePaidInvoice(Mage_Sales_Model_Order $order, $transaction_id = null)
    {
        ...
        if (Mage::getStoreConfig("payment/ginger_creditcard/send_invoice_mail", $order->getStoreId())) {
            $invoice->sendEmail();
        }
