 ...
 *
 **/
class Ginger_Ginger_BancontactController extends Mage_Core_Controller_Front_Action
{
    /**
     * @var Ginger_Ginger_Model_bancontact
     */
    ...
    {
        $this->_bancontact = Mage::helper('ginger/bancontact');
    ...
    public function paymentAction()
    {
        ...
        try {
            ...
            $description = str_replace('%', $order_id,
                Mage::getStoreConfig("payment/ginger_bancontact/description", $order->getStoreId())
            );
            $return_url = Mage::getUrl('ginger/bancontact/return');
            $customer = $this->_getCustomerData($order);
            $currency = $order->getOrderCurrencyCode();

            if ($this->_bancontact->createOrder($order_id, $amount, $currency, $description, $return_url, $customer)) {
                if (!$order->getId()) {
                ...
                $payment->setIsTransactionClosed(false)
                    ->setGingerOrderId($this->_bancontact->getOrderId());

                // Sets the above transaction
                $order->setPayment($payment);
                $order->setGingerOrderId($this->_bancontact->getOrderId());
                $order->save();

                $payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_AUTH);

                $pendingMessage = Mage::helper('ginger')->__(Ginger_Ginger_Model_bancontact::PAYMENT_FLAG_PENDING);
    ...
    public function webhookAction()
    {
        ...
        if (in_array($input['event'], array("status_changed"))) {
            ...
            $ginger_order = $this->_bancontact->getOrderDetails($ginger_order_id);
            ...
            try {
                if ($order->getData('status') == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                    ...
                    $order_amount_cents = Ginger_Ginger_Helper_Data::getAmountInCents($order->getGrandTotal());
                        ...
                        case "completed":
                            ...
                            if ($amount_paid_cents == $order_amount_cents) {
                                ...
                                $order->setState(
                                    Mage_Sales_Model_Order::STATE_PROCESSING,
                                    Mage_Sales_Model_Order::STATE_PROCESSING,
                                    Mage::helper('ginger')->__(Ginger_Ginger_Model_Bancontact::PAYMENT_FLAG_COMPLETED),
                                    true
                                );
                                $order->save();
                                ...
                                if (Mage::getStoreConfig("payment/ginger_bancontact/send_invoice_mail",
                                    $order->getStoreId())
                                ) {
                                ...
                                // Create invoice.
                                if (Mage::getStoreConfig("payment/ginger_bancontact/generate_invoice_upon_completion",
                                    $order->getStoreId())
                                ) {
                                ...
                                $order->setState(Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW,
                                    ...
                                    Mage::helper('ginger')->__(Ginger_Ginger_Model_Bancontact::PAYMENT_FLAG_FRAUD),
                                    false)->save();
                            }
                            break;
                        case "cancelled":
                            ...
                            $order->setState(
                                ...
                                Mage_Sales_Model_Order::STATE_CANCELED,
                                Mage::helper('ginger')->__(Ginger_Ginger_Model_Bancontact::PAYMENT_FLAG_CANCELLED),
                                true
                            );
                            $order->save();
                            break;
    ...
    public function returnAction()
    {
        ...
        try {
            if (!empty($order_id)) {
                $ginger_order_details = $this->_bancontact->getOrderDetails($order_id);
    ...
    protected function _savePaidInvoice(Mage_Sales_Model_Order $order, $transaction_id = null)
    {
        ...
        if (Mage::getStoreConfig("payment/ginger_bancontact/send_invoice_mail", $order->getStoreId())) {
            $invoice->sendEmail();
        }
